...
class Hunk {
	...
	validateLines(linesDict: { [key: number]: string }, problems: string[]) {
		...
		while (hunkInd < this.lines.length && fileInd <= maxLineNumber) {
			...
			if (line.type === 'add') {
				...
				const linesBehind = this.lines.slice(0, hunkInd)
				if (
				...
				if (
					...
					if (line.type === 'remove') {
						let matchingAddLineIndex: number
	...
	private checkStartLine(linesDict: { [key: number]: string }): boolean {
		return isSimilar(this.lines[0].content, linesDict[this.startLinePreEdit])
	}

	private findStartLine(
		linesDict: { [key: number]: string },
		problems: string[],
	): boolean {
		for (let i = -50; i <= 50; i++) {
			const adjustedStartLine = this.startLinePreEdit + i
			if (
				linesDict[adjustedStartLine] &&
				isSimilar(this.lines[0].content, linesDict[adjustedStartLine])
			) {
				this.startLinePreEdit = adjustedStartLine
				this.startLinePostEdit += i
				problems.push(`Hunk start line adjusted by ${i} lines`)
				return true
			}
		}
		problems.push(`Could not find a matching start line for hunk`)
		return false
	}
	...
	private addRetainedLine(line: string, index: number): void {
		this.lines.splice(index, 0, { type: 'retain', content: line })
		this.categoryCounts.retain++
	}

	private relabelLine(index: number, newType: LineType): void {
		const oldType = this.lines[index].type
		this.lines[index].type = newType
		this.categoryCounts[oldType]--
		this.categoryCounts[newType]++
	}
